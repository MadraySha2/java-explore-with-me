...
public interface StatsRepository extends JpaRepository<Hit, Long> {
    ...
    @Query("select new ru.practicum.server.model.OutStats(h.app, h.uri, count(h.ip)) " +
            "from Hit h " +
    ...
    @Query("select new ru.practicum.server.model.OutStats(h.app, h.uri, count(distinct h.ip)) " +
            "from Hit h " +
            "where h.timestamp between ?1 and ?2 " +
            "and h.uri in (?3) " +
            "group by h.app, h.uri " +
